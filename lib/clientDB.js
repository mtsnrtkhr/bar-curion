...
async function initDb(url) {
  ...
  export async function mergeRecipesAndIngredients(recipeDb) {
    const cocktails = recipeDb.data.cocktails;
    const ingredients = recipeDb.data.ingredients;

    const mergedRecipes = cocktails.map(recipe => ({
      ...recipe,
      ingredients: recipe.ingredients.map(ing => {
        ...
        if (ingredient) {
          return {
            ...ing,
            ...ingredient,
            products: ingredient.products.map(p => ({
              ...p,
...
export async function initClientDB() {
    ...
    if (!mergedRecipeDb) {
      mergedRecipeDb = await mergeRecipesAndIngredients(recipeDb)
    }
}
...
export function getMergedRecipeDB() {
  return mergedRecipeDb
}
...
export async function searchRecipes(query, isAdvancedSearch = false) {
  ...
  if (!isAdvancedSearch) {
    ...
    // 高度な検索のロジック
    try {
      const recipeIds = await searchRecipesAdvanced(query, mergedRecipeDb);
      console.log(recipeIds);
      const recipePromises = Array.from(recipeIds).map(id => getRecipeById(id));
