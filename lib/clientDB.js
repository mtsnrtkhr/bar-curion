...
import { JSONFile } from 'lowdb/browser'
...
async function initDb(url) {
    ...
    try {
      ...
      console.log("Raw response:", text) // レスポンスの内容をログ出力
      console.log("Raw response (first 100 characters):", text.slice(0, 100))
      console.log("Raw response length:", text.length)
      const initialData = JSON.parse(text) // テキストを JSON としてパース
      db.data = initialData
      await db.write()
    } catch (error) {
...
export async function getRecipeById(id) {
  ...
  //await initClientDB()
  return recipeDb.data.cocktails.find(recipe => recipe.id === parseInt(id))
}
