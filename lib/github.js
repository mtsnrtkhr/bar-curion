...
export async function fetchRecipesData() {
  const response = await fetch(getAssetPath('/data/recipes.json'));
  if (!response.ok) {
...
export async function fetchGraphData() {
  const response = await fetch(getAssetPath('/data/graph-data.json'));
  if (!response.ok) {
...
export async function getRecipes() {
  ...
  const { data } = await octokit.repos.getContent({
    ...
    path: getAssetPath('/data/recipes.json'),
  });
