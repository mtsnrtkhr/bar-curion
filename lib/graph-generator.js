...
async function generateGraphData() {
  try {
    const recipesData = JSON.parse(await fs.readFile(path.join(process.cwd(), 'data', 'recipes.json'), 'utf8'));
    ...
    await fs.writeFile(path.join(process.cwd(), 'data', 'graph-data.json'), JSON.stringify(graphData, null, 2));
